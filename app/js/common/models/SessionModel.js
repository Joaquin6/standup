define(['jquery', 'underscore', 'backbone', 'main', 'moment', 'helpers/utility', 'helpers/events', 'models/UserModel', 'services/io'], function($, _, Backbone, Main, Moment, Utility, Events, UserModel, io) {
    var sessionModel = Backbone.Model.extend({
        ...
        initialize: function() {
            _.bindAll(this, '__countdown', '__onSuccessfulLogin', '__onFailedLogin');
            var siteDesc = Main.App.SiteDesc || null;
            this.set('siteType', siteDesc);
            var sessionCookie = Utility.getPersistence('fsSession') || null;
            var userCookie = Utility.getPersistence('fsUser') || null;
        ...
        initializeSession: function(params) {
            Utility.setPersistence('fsSession', params);
        ...
        login: function(payload) {
            Utility.setPersistence('fsLogin', payload.email);
            if (!Utility.isProductionEnv())
                console.time("LOGIN API CALL");
            io.POST('/login', payload, this.__onSuccessfulLogin, this.__onFailedLogin);
        },
        logout: function(urlLink) {
            this.stopListening();
            Utility.removePersistence('fsSession');
            Utility.removePersistence('fsUser');
            this.clear();
        ...
        getAuth: function(options, callback) {
            if (options.preamble)
                io.storePreamble(options.preamble);
            var cookie = Utility.getPersistence('fsSession');
            if (cookie == undefined) {
                this.clear();
                callback();
            } else if (cookie.token) {
                this.set('authenticated', true);
                var accnt = Utility.getPersistence('fsUser');
                if (accnt)
        ...
        __onSuccessfulLogin: function(data, status, xhr) {
            ...
            if (res.data.statusCode) {
                ...
                if (res.data.statusCode === 401) {
                    ...
                    Utility.removePersistence('fsLogin');
                }
            }
        ...
        __onFailedLogin: function(xhr, status, error) {
            var res = {};
            res.status = status;
            res.data = {};
