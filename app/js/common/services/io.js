define(['jquery', 'underscore', 'backbone', 'main', 'router', 'helpers/events', 'helpers/utility'], function($, _, Backbone, Main, Router, Events, Utility) {
	return {
		...
		GET : function(resource, successCB, errorCB, options) {
			var baseAddress = this.__getBaseAddress(options);
		...
		storePreamble: function(preamble) {
			preamble = '/' + preamble;
			if (!this.__preamble || this.__preamble !== preamble) {
				this.__preamble = preamble;
				if (!Utility.isProductionEnv())
					console.log('IO: Preamble Set => ' + preamble);
			}
		},
		__post : function(type, resource, payload, successCB, errorCB) {
			var baseAddress = this.__getBaseAddress(payload);
		...
		__getBaseAddress: function(payload) {
			if (!this.__preamble)
				this.storePreamble(Main.App.Preamble);
			if (payload && payload.excludePreamble) {
				delete payload.excludePreamble;
				return Utility.getProtocol() + '//' + Utility.getHost();
			}
			return Utility.getProtocol() + '//' + Utility.getHost() + this.__preamble;
		},
