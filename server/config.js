...
function requireConfig(configDir, version) {
	...
	console.info('%s settings from %s', version, verPath);
	try {
		config = require(verPath);
	}
	catch (err) {
		console.error(util.format('Couldn\'t find or module doesn\'t export environmental config for "%s"@%s in %s - returning empty\n', version, verPath, configDir), err);
		config = {};
	}
	return config;
}
...
function config(configDir, env) {
    ...
    console.info('Getting config settings from %s w/ env: %s', configDir, env);
    ...
    if (process.env.VERBOSE) {
        console.log('Running with configurations...\n', util.inspect(settings, {
            depth: null,
            colors: true
        }));
    }
...
function customizeDockerLinks(link, key, val, envVars) {
	...
	if (link + '_NAME' === key) {
		...
		envVars[name].host = _.chain(val.split('/'))
...
function mergeSettings() {
	...
	if (!precedence || !precedence.length) return {};

	return _.chain(precedence)
			.filter(function (p) {
				return p;
			})
			.map(function (p) {
...
function defaultSettingsMerge(config, program) {
	return mergeSettings(config, function () { return dockerEnv(config.allowable ? config.allowable() : undefined); }, program);
}
